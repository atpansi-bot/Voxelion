...
public sealed class VoxelionGame : Game
{
    ...
    public VoxelionGame()
    {
        ...
        // Do not hardcode resolution on Android — let the activity surface size the backbuffer
    ...
    {
        try { _input.Update(); }
        catch { /* TouchPanel can fail on early Android frames */ }

        try { _currentScene?.Update(gameTime, _input.Current); }
        catch { /* scene error must not stop the loop */ }
    ...
    protected override void Draw(GameTime gameTime)
    {
        ...
        try
        {
            ...
            {
                // Fallback so the screen is never pure black
